...
//refactor code for adventrurer, smithy,great hall, village and council room  cards
//int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
void adventurerEffect(struct gameState *state, int handPos){
  ...
  if(handCard(handPos,state) == adventurer){
      discardCard(handPos,Player,state, 0);
  }else{
      printf("Hand Position Not Adventurer!!\n");
  }
 //     return 0;
...
void great_hallEffect(struct gameState *state, int handPos){
  int currentPlayer = whoseTurn(state);
	  //+1 Card
      drawCard(currentPlayer, state);
...
//  int z = 0;// this is the counter for the temp hand
  ...
  switch( card ) 
    {
    case adventurer:
	//refactored adventurer function, which returns int
    ...
    case smithy:
	//refactored smithy card function, which return int
	smithyEffect(state, handPos);
	return 0;
		
    case village:
	//refactored village card function, which return int
